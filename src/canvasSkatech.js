...
import { Image, StyleSheet, Text, TouchableOpacity, View, Dimensions, ScrollView, FlatList, Platform, Alert } from 'react-native'
...
export default function DrawOnImage() {
    ...
    console.log("hkuhkhkukukukukukukuku", imagepath,closedImgaePath)
    const openCamera = () => {
        let options = {
            storageOptions: {
                skipBackup: true,
    ...
    const onEditImageClick = () => {
        ...
        setImagePath(imagepath.substring(5)),
        setShowCanvasModel(true)
   ...
   const canvasBackPress = () =>{
    ...
    return (
        <>
            <TouchableOpacity style={styles.photoText} onPress={() => [setShowImageMenu(true)]}>
...

            {
                imagepath ?
                    <>
                        <TouchableOpacity onPress={() => setViewImage(true)} style={{ width: '100%', height: '70%', marginTop: 50, alignItems: 'center' }} >
                            <Image source={{ uri: imagepath }} style={{ width: '100%', height: '100%' }} resizeMode="stretch" />
                        </TouchableOpacity>
                    </>
                    : console.log("hhhjjj")
            }
                ...
                viewImage && (
                    ...
                    >
                        <View style={{ flexDirection: 'row', }}>
                            ...
                            <TouchableOpacity onPress={() => ImgaeModelFunction()} style={{ flexDirection:'row'}}>
                               
                                <Icon name="camera" size={22} color={'white'} style={{}}/>
                                <Text style={{ color: 'white',marginLeft:10}}>Select image</Text>
                            </TouchableOpacity>
                            </View>
                            <View style={{ width:'50%',justifyContent:'flex-end',alignItems:'flex-end' }}>
                                <TouchableOpacity onPress={() => onEditImageClick()} style={{ flexDirection: 'row',marginRight:15 }}>

                                    <Icon name='edit' size={26} color={'white'} />
                                    <Text style={{ alignSelf: 'center', color: 'white' }}>Edit Image</Text>

                                </TouchableOpacity>
                            </View>

                        </View>
                        <Image source={{ uri: imagepath }} style={{ width: '100%', height: '100%', alignSelf: 'center' }} resizeMode="stretch" />
                ...
                showCanvasModel && (
                    ...
                    >
                        ...
                        <RNSketchCanvas
                            ...
                            closeComponent={<View style={styles.functionButton} onTouchStart={() => closeCanvasFunction()}><Text style={{ color: 'white' }}>Close</Text></View>}
                            undoComponent={<View style={styles.functionButton} onTouchStart={() => setSelection(false)}><Text style={{ color: 'white' }}>Undo</Text></View>}
                            clearComponent={<View style={[styles.functionButton]} onTouchStart={() => setSelection(false)}><Text style={{ color: 'white' }}>Clear</Text></View>}
                            eraseComponent={<View style={isSelected ? styles.functionButton1 : styles.functionButton} onTouchStart={() => setSelection(true)}><Text style={{ color: 'white' }}>Eraser</Text></View>}
                            strokeComponent={color => (
                                <View style={[{ backgroundColor: color }, styles.strokeColorButton]} />
                            )}
                            strokeColors={[{ color: '#000000' }, { color: '#FF0000' }, { color: '#008000' }, { color: '#0000FF' }, { color: '#FFFF00' }, { color: '#FF69B4' }]}
                            ...
                            strokeSelectedComponent={(color, index, changed) => {
                                ...
                                return (
                                    <View style={[{ backgroundColor: color, borderWidth: 3, margin: 20 }, styles.strokeColorButton]} />
                                )
                            }}
                            ...
                            onSketchSaved={(success, filePath) => { [console.log("success success success", success), setImagePath("file://" + filePath), setShowCanvasModel(false), setSelection(false)] }}
...
const styles = StyleSheet.create({
    ...
    strokeColorButton: {
        marginHorizontal: 2.5, marginVertical: 8, width: 30, height: 30, borderRadius: 15, borderColor: 'white'
    },
    strokeWidthButton: {
    ...
    functionButton1: {
        marginHorizontal: 2.5, marginVertical: 8, height: 30, width: 60,
        backgroundColor: '#39579A', justifyContent: 'center', alignItems: 'center', borderRadius: 5, borderColor: 'red', borderWidth: 2
    },
})